...
function createHomeProjectContainer(list) {
    ...
    li.innerHTML = `
        <figure class="card__img-cont">
            <img src="${list.bg_image}" alt="" class="card__img">
        </figure>
        <div class="card__overlay">
            <h3 class="card__title">${list.name}</h3>
            <p class="card__description">${list.summary}</p>
            <div class="card__link">
                <a href="${
                    list.type == "personal" ? "" : list.github
                }" title="github code" target="blank">
                ...
                <a href="${list.live}" title="live" target="blank">
                    <figure aria-hidden="true">
...
function loadLists() {
    for (let i = 0; i < 3; i++) {
        let li = createHomeProjectContainer(allProjects[i]);
        homeProjectCont.append(li);
    }
}

loadLists();

homeProjectCont.addEventListener("focusin", (event) => {
    let focusCard = event.target.closest(".card");
    if (!focusCard) return;
    homeProjectCont
        .querySelectorAll(".card")
        .forEach((card) => card.classList.remove("card__focus"));
    focusCard.classList.add("card__focus");
});
